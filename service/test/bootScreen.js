...
// What the service's HTML hands the page.
const CONFIG = JSON.parse(/window\.TUBE_BOOT = (\{[^<]*\});/.exec(html(''))[1]);
...
const run = (script, replies, turns, options) => {
    ...
    XMLHttpRequest.prototype.send = function send() {
        ...
        if (this.url.indexOf('/__tube/ping') !== -1) {
...
const checks = (script) => {
    ...
    check('it tells the service how long it waited, once', handed.asked.filter((url) => url.indexOf('waited=') !== -1).length === 3
        && handed.asked.slice(3).every((url) => url.indexOf('waited=') === -1), handed.asked.join(' '));
    ...
    check('its policy lets it reach the service and YouTube, and go there',
        /connect-src http:\/\/127\.0\.0\.2:8099 https:\/\/www\.youtube\.com;/.test(page) && /h5vcc-location-src https:\/\/www\.youtube\.com/.test(page));
    ...
    check('it is written where file:///tube/boot.html resolves',
        written === path.join(scratch, 'web', 'tube', 'boot.html') && fs.readFileSync(written, 'utf8') === page);
