...
// There is no meaningful load event — the script is evaluated into a page that builds
// itself afterwards — so a <video> element is the cheapest signal the player exists.
...
function start() {
  ...
  // The guide is built before this script runs, so the trimmed sidebar only appears once
  // YouTube is asked to build it again.
  reloadGuide();
}

// The resolver singleton can arrive after the player element. Keep asking, but stop
// eventually: an endless interval is a frame cost for the life of the session.
...
// YouTube's own stylesheet carries a nonce, so appending to it keeps everything under one
// CSP-approved element.
function addStyles() {
...
function liftLowEndRestrictions() {
  ...
  // YouTube puts the class back whenever it re-renders, so it has to be removed on every
  // attribute change rather than once at startup.
  try {
...
// A <video> element exists while YouTube's first-run flow is on screen, so the signal
// `start()` waits on says nothing about whether YouTube is finished with it.
