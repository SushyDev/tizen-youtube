...
## What the enhanced player holds

It used to write every segment to the television's storage, and viewers ran out of space.
The reason was counting: `READ_AHEAD` 10 plus `KEEP_BEHIND` 15 is twenty-five segments, and
a segment is five seconds — which is 5MB at 720p and **eighteen at 2160p60**. Counting
segments lets the bitrate decide the footprint, and at 2160p60 it decided ~450MB, twice
over when a quality change opened a second session.

What YouTube's own client does instead, read off the wire through this proxy: it fetches
about **916KB at a time** (250KB–2.8MB over twenty-four requests) and appends each piece to
a `SourceBuffer`. It never treats a segment as a unit. So its buffer holds *seconds*, never
*whole segments*, and the bitrate changes how often it asks rather than how much it keeps.

Samsung's own limits, from `media/base/demuxer_memory_limit.h` on the Tizen branch matching
this set — these are not upstream Chromium's, which the fork replaces outright:

```
kDemuxerStreamVideoMemoryLimitTvUHD      80 MiB      (evicts at 60 — the soft limit is 3/4)
kDemuxerStreamVideoMemoryLimitTvDefault  40 MiB      (<= 1080p)
kDemuxerStreamAudioMemoryLimitTvDefault   8 MiB
```

Branched on resolution alone, and it latches upward: once a UHD config is seen it stays at
80 MiB for the life of the process. There is also a Tizen-only refusal to accept an append
at all once ~20s is buffered ahead, which returns `QuotaExceededError` where upstream would
quietly evict. So the budget to match here is **60–88 MiB**, not the 30 MiB that upstream
Chromium's low-memory tier would suggest.

**So the window is counted in bytes.** `WINDOW_BYTES` is what may be held at or ahead of
the player and `TAIL_BYTES` what is kept behind it, and segments are readable *as they
arrive* rather than once complete — a reader waits on a byte high-water mark instead of on
a finished file. That last part is what makes a window this size playable at all: an
eighteen-megabyte segment is worth nothing until it is whole, so half of any segment-shaped
buffer is dead weight on average.

Nothing is written to disk. There is no code left that could.

```
2160p60 HDR, playing:  74.9-102.0 MB held, 0 on disk, 1.02x realtime, 24-28s buffered ahead
```

Three things went wrong on the way, each worth not repeating:

**The gate must measure the read-ahead, not everything held.** Counting the tail too meant
a tail large enough to be useful kept the gate shut, and the download starved fetching the
very thing it was waiting for.

**Eviction has to happen where the player is, not in the download loop.** With the window
counted in segments, `satisfied` moved when the *player* did, so the loop could always make
progress. Counted in bytes, the only thing that frees bytes is eviction — and putting that
inside the loop, behind the gate the eviction exists to open, deadlocks: full window,
blocked loop, nothing released, nothing fetched, ten seconds of video and then silence.
It lives in `want()` now, which is the player telling us where it is.

**A tail must be big enough for one real rewind.** Six megabytes is a third of one 2160p60
segment, so every re-ask missed. The platform's player re-asks constantly — a resume hands
it `#t=`, the fragment never reaches the server, so it starts at zero and then jumps — and
a segment that has been dropped and asked for again restarts the whole stream, measured at
four seconds of nothing each time. Forty megabytes is a little over two segments at
2160p60, and forty at 720p.
...
### What a launch costs, and where it goes

Every launch writes one line to `/home/owner/share/tube/service.log` — `boot: …` — because
the boot screen is replaced by YouTube a frame after it works its timings out, and they
have to survive that to be readable at all. That log outlives the app, rolls at 64KB, and
is readable from another machine without a debug build, which is what makes a release
launch measurable. It is the only reason any of the numbers below exist.

```
warm   shell 0.063s, keys 0.014s, service 0.224s, 1 ask, total 0.227s
cold   shell 0.063s, keys 0.015s, service 2.259s, 1 ask, total 2.261s
```

**Warm** is the ordinary launch: the service is still listening from last time and the
first ask answers it. **Cold** is after a reinstall, a reboot, or the package being killed,
and splits into **~1.8s of Tizen dispatching the app control and starting a Node process**
— before the service's own first line runs — and **~0.6s** of the service reaching
`app.listen`. Only the second half is ours, and the whole bundle takes 82ms to listen on a
developer machine, so there is little in it to win.

Four things that looked like they mattered and do not, each measured rather than reasoned
about:

| | |
| --- | --- |
| the twelve `tvinputdevice.registerKey` calls | **14ms** |
| `/__tube/state` reading and SHA-256ing the bundle | **5ms**, of a 17ms handler |
| the boot screen's own work, all of it | **63ms**, and it overlaps the wait |
| the shell's first request going out late | it goes out **17ms** in |

The shell asks *before* it launches anything, which is the other half of why a warm launch
costs a fifth of a second. Launching first meant paying that ~1.8s dispatch on every start,
including the ones where the service was already up and nothing needed launching at all.

### Being told instead of asking

A cold launch used to poll `/__tube/state` on a 200ms grid — fifteen asks. It takes **one**
now: the service sends its state over a Tizen message port from inside the `app.listen`
callback, and the boot screen is sitting on that port before it launches anything. The
state travels in the message, so there is nothing left to ask for afterwards.

Sent from the listen callback and **nowhere earlier**. "The service started" and "the
socket is bound" are different claims, and the shell's next act is to navigate to that
socket.

**Trusted message ports do not work on this television.**
`requestTrustedLocalMessagePort` registers, `requestTrustedRemoteMessagePort` finds it,
`sendMessage` returns without throwing — and the message is never delivered. Neither end
reports anything wrong; only instrumenting *received* separately from *sent* shows it. The
untrusted pair (`requestLocalMessagePort` / `requestRemoteMessagePort`) delivers, which is
what Samsung's own TV service sample uses. Both are registered and both are sent to, since
that finding is one set and the trusted pair costs nothing.

The poll is kept behind it on a 2.5s backstop, so a set that never delivers an announcement
still boots, bounded by the same 20s deadline as before.

Everything the service does there is inside one `try`, `getAppInfo` and the `tizen`
namespace included. `requestRemoteMessagePort` throws `NotFoundError` whenever nobody is
waiting — the ordinary case for a DIAL cast launch, and for one where the shell has already
handed over — and an uncaught throw in that process is postmortem's `process.exit(1)`. No
announcement is a slower launch; a throw there is no launch at all.

### auto-restart and on-boot are accepted and inert

`<tizen:service auto-restart="true" on-boot="true">` installs cleanly and does nothing.
After a power cycle the first launch still had to start the service — 74 asks, with the
service's `started:` line arriving fourteen seconds *after* the boot screen began asking.
Tizen's own documentation says these have not been supported on every device since 2.4, and
this is one of the sets where they are not. They were taken out again rather than left in:
a service asked to run from boot holds memory for no benefit here, and the app is trying to
stay resident in that same memory.

That is the only lever there was at the ~1.8s. The platform's cost is paid when the service
starts, and the only way not to pay it during a launch is for the service to have started
already. `background-support="enable"` does keep it alive *between* launches, which is why
the warm path is the common one — but nothing brings it back after a reboot except opening
the app.

### The launch that should not have happened at all

None of the above matters if the app never had to launch. It did, every time: Home and back
re-ran the boot screen and landed on the YouTube home page rather than where the viewer
was, where the stock app resumes.

`multitasking.support` was `false`. Samsung's documentation is unambiguous — with it false
or absent the platform **terminates** the app when it loses focus instead of hiding it, and
resume is not available. It arrived in `3e06f32` beside `voice.support`, taken from
Samsung's NativePlayer sample as a rendering-priority hint; it is not one. The frame drops
it was reaching for were in the media path rather than the renderer, so it bought nothing
and cost every single return to the app.

Set to `true`, Home and back resumes. That is worth more than every millisecond in this
section put together, because the fastest launch is the one that does not happen.

`config.xml` also carried **two** widget-level `<tizen:setting>` elements, one saying
`background-support="disable"` and a later one saying `enable`. The second had never done
anything: `tizen:setting` may occur once, and W3C Widget Packaging — which Tizen's parser
follows — ignores a repeated singleton rather than merging it, so the first is the whole of
it. That the app was terminated on focus loss rather than hidden agrees with the first one
being the live one. The duplicate is removed; nothing about the package's behaviour changes
with it, and the setting that decides this on Samsung's platform is
`multitasking.support` regardless.
