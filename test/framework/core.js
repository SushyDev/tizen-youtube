...
const { register, boot, booted, PHASES } = await import('../../framework/registries/register.js');
const { every, after, until, stop, running } = await import('../../framework/registries/schedule.js');
const { onKey } = await import('../../framework/registries/keys.js');
...
await asyncCheck('a feature that throws does not stop the others', async () => {
    const fresh = await import('../../framework/registries/register.js?throws');
    const order = [];
...
check('framework/index.js fronts every module under it', () => {
    ...
    // Every .js file under framework/ except its own index.js and the vendored sha256.
    const filesUnder = (sub) => readdirSync(join(dir, sub), { withFileTypes: true })
        .flatMap((entry) => (entry.isDirectory()
            ? filesUnder(join(sub, entry.name))
            : [join(sub, entry.name)]))
        .filter((relative) => relative.endsWith('.js'))
        .filter((relative) => relative !== 'index.js' && relative !== 'vendor/tiny-sha256.js');

    const missing = filesUnder('.')
        .flatMap((relative) => exportedBy(readFileSync(join(dir, relative), 'utf8'))
            .filter((name) => PRIVATE.indexOf(name) === -1)
            .filter((name) => !new RegExp(`\\b${name}\\b`).test(surface))
            .map((name) => `${relative} exports ${name}`));
